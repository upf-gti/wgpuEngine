...
struct OpenXRContext : public XRContext {
    ...
    void apply_haptics(uint8_t controller, float amplitude, float duration) override;
    void stop_haptics(uint8_t controller) override;
    ...
    uint32_t swapchain_length = 0; // Number of textures per swapchain
    ...
    Error check_backend_requirements();

#if defined(BACKEND_VULKAN)
    Error check_vulkan_version(XrGraphicsRequirementsVulkanKHR* vulkan_reqs);
#elif defined(BACKEND_DX12)
    Error check_dx12_version(XrGraphicsRequirementsD3D12KHR* dx12_reqs);
#endif

private:
    Error create_action(XrActionSet actionSet,
            XrPath* paths,
            uint32_t num_paths,
