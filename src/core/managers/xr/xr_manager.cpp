...
WGPUTextureView XRManager::get_swapchain_view(uint8_t eye_idx, uint32_t image_idx)
{
    return xr_context ? xr_context->get_swapchain_view(eye_idx, image_idx) : nullptr;
}

uint32_t XRManager::get_num_images_per_swapchain()
{
    return xr_context ? xr_context->get_num_images_per_swapchain() : 2;
}
