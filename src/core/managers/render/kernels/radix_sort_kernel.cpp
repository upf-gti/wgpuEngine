...
RadixSortKernel::RadixSortKernel(WGPUBuffer keys_buffer, uint32_t keys_byte_size, WGPUBuffer values_buffer, uint32_t values_byte_size, uint32_t count, uint32_t bit_count, const glm::uvec2& workgroup_size) :
        keys_buffer(keys_buffer), keys_byte_size(keys_byte_size), values_buffer(values_buffer), values_byte_size(values_byte_size), count(count), bit_count(bit_count), workgroup_size(workgroup_size)
{
    ...
    block_sum_shader = RenderStorage::get_singleton()->get_shader_from_source(shaders::radix_sort::source, shaders::radix_sort::path, shaders::radix_sort::libraries);
    reorder_shader = RenderStorage::get_singleton()->get_shader_from_source(shaders::radix_sort_reorder::source, shaders::radix_sort_reorder::path, shaders::radix_sort_reorder::libraries);
...
{
    dispatch_size = find_optimal_dispatch_size(workgroup_count);
}

void RadixSortKernel::create_prefix_sum_kernel()
{
    prefix_block_sum_buffer = RenderAPI::get_singleton()->buffer_create(prefix_block_workgroup_count * 4,
...
RadixSortKernel::PipelineData* RadixSortKernel::create_block_sum_pipeline(WGPUBuffer in_keys, uint8_t bit)
{
    ...
    pipeline_data->bind_group = RenderAPI::get_singleton()->bind_group_create(uniforms, block_sum_shader, 0, "radix_sort_block_sum_bind_group");
...
RadixSortKernel::PipelineData* RadixSortKernel::create_reorder_pipeline(WGPUBuffer in_keys, WGPUBuffer in_values, WGPUBuffer out_keys, WGPUBuffer out_values, uint8_t bit)
{
    ...
    pipeline_data->bind_group = RenderAPI::get_singleton()->bind_group_create(uniforms, reorder_shader, 0, "radix_sort_reorder_bind_group");
...
{
    tmp_keys_buffer = RenderAPI::get_singleton()->buffer_create(keys_byte_size,
            WGPUBufferUsage_Storage | WGPUBufferUsage_CopySrc | WGPUBufferUsage_CopyDst, nullptr,
            "radix_sort_tmp_keys");

    tmp_values_buffer = RenderAPI::get_singleton()->buffer_create(values_byte_size,
            WGPUBufferUsage_Storage | WGPUBufferUsage_CopySrc | WGPUBufferUsage_CopyDst, nullptr,
            "radix_sort_tmp_values");

    local_prefix_sum_buffer = RenderAPI::get_singleton()->buffer_create(count * sizeof(uint32_t),
            WGPUBufferUsage_Storage | WGPUBufferUsage_CopySrc | WGPUBufferUsage_CopyDst, nullptr,
