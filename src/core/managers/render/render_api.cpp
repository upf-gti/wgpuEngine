...
#endif
        cubemap_mipmap_pipeline.mipmap_shader = RenderStorage::get_singleton()->get_shader_from_source(shaders::cubemap_downsampler::source, shaders::cubemap_downsampler::path, shaders::cubemap_downsampler::libraries, defines);
...
RenderAPI::sMipmapPipeline RenderAPI::get_mipmap_pipeline(WGPUTextureFormat texture_format)
{
    ...
    Shader* shader = RenderStorage::get_singleton()->get_shader_from_source(shaders::mipmaps::source, shaders::mipmaps::path, shaders::mipmaps::libraries, { custom_define });
...
WGPUCommandEncoder RenderAPI::get_main_command_encoder()
{
    return command_encoder;
}

WGPUQueue RenderAPI::get_main_queue()
{
    return device_queue;
}

WGPUSurface RenderAPI::get_main_surface()
{
    return surface;
}

WGPURenderPassEncoder RenderAPI::begin_render_pass(WGPUCommandEncoder commandEncoder, WGPURenderPassDescriptor const* descriptor)
{
    return wgpuCommandEncoderBeginRenderPass(commandEncoder, descriptor);
}

void RenderAPI::render_pass_set_bind_group(WGPURenderPassEncoder renderPassEncoder, uint32_t groupIndex, WGPUBindGroup group, size_t dynamicOffsetCount, uint32_t const* dynamicOffsets)
{
    wgpuRenderPassEncoderSetBindGroup(renderPassEncoder, groupIndex, group, dynamicOffsetCount, dynamicOffsets);
}

void RenderAPI::render_pass_encoder_set_vertex_buffer(WGPURenderPassEncoder renderPassEncoder, uint32_t slot, WGPUBuffer buffer, uint64_t offset, uint64_t size)
{
    wgpuRenderPassEncoderSetVertexBuffer(renderPassEncoder, slot, buffer, offset, size);
}

void RenderAPI::render_pass_encoder_set_index_buffer(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer buffer, WGPUIndexFormat format, uint64_t offset, uint64_t size)
{
    wgpuRenderPassEncoderSetIndexBuffer(renderPassEncoder, buffer, format, offset, size);
}

void RenderAPI::render_pass_encoder_draw(WGPURenderPassEncoder renderPassEncoder, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance)
{
    wgpuRenderPassEncoderDraw(renderPassEncoder, vertexCount, instanceCount, firstVertex, firstInstance);
}

void RenderAPI::render_pass_encoder_draw_indexed(WGPURenderPassEncoder renderPassEncoder, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int32_t baseVertex, uint32_t firstInstance)
{
    wgpuRenderPassEncoderDrawIndexed(renderPassEncoder, indexCount, instanceCount, firstIndex, baseVertex, firstInstance);
}

void RenderAPI::render_pass_encoder_draw_indexed_indirect(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer indirectBuffer, uint64_t indirectOffset)
{
    wgpuRenderPassEncoderDrawIndexedIndirect(renderPassEncoder, indirectBuffer, indirectOffset);
}

void RenderAPI::render_pass_encoder_draw_indirect(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer indirectBuffer, uint64_t indirectOffset)
{
    wgpuRenderPassEncoderDrawIndirect(renderPassEncoder, indirectBuffer, indirectOffset);
}

void RenderAPI::render_pass_encoder_end(WGPURenderPassEncoder renderPassEncoder)
{
    wgpuRenderPassEncoderEnd(renderPassEncoder);
}

void RenderAPI::render_pass_encoder_release(WGPURenderPassEncoder renderPassEncoder)
{
...
void RenderAPI::cubemap_mipmaps_create(WGPUTexture texture, WGPUExtent3D texture_size, uint32_t mip_level_count, WGPUTextureViewDimension view_dimension, WGPUTextureFormat format, WGPUOrigin3D origin, WGPUCommandEncoder custom_command_encoder)
{
    ...
    push_debug_group(compute_pass, "Create Cubemap Mipmaps");
...
WGPUPipelineLayout RenderAPI::create_pipeline_layout(const std::vector<WGPUBindGroupLayout>& bind_group_layouts, char const* label)
{
    ...
    layout_descr.label = get_string_view(label);
...
WGPUQuerySet RenderAPI::create_query_set(uint8_t maximum_query_sets, char const* label)
{
    ...
    query_set_descriptor.label = get_string_view(label);

    return wgpuDeviceCreateQuerySet(device, &query_set_descriptor);
}

void RenderAPI::push_debug_group(WGPURenderPassEncoder render_pass, char const* label)
{
#ifndef __EMSCRIPTEN__
    wgpuRenderPassEncoderPushDebugGroup(render_pass, get_string_view(label));
#endif
}

void RenderAPI::push_debug_group(WGPUComputePassEncoder compute_pass, char const* label)
{
#ifndef __EMSCRIPTEN__
    wgpuComputePassEncoderPushDebugGroup(compute_pass, get_string_view(label));
#endif
}
