...
public:
    ...
    WGPUCommandEncoder get_main_command_encoder();
    WGPUQueue get_main_queue();
    WGPUSurface get_main_surface();

    // RenderPasses
    WGPURenderPassEncoder begin_render_pass(WGPUCommandEncoder commandEncoder, WGPURenderPassDescriptor const* descriptor);
    void render_pass_set_bind_group(WGPURenderPassEncoder renderPassEncoder, uint32_t groupIndex, WGPUBindGroup group, size_t dynamicOffsetCount, uint32_t const* dynamicOffsets);
    void render_pass_encoder_set_vertex_buffer(WGPURenderPassEncoder renderPassEncoder, uint32_t slot, WGPUBuffer buffer, uint64_t offset, uint64_t size);
    void render_pass_encoder_set_index_buffer(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer buffer, WGPUIndexFormat format, uint64_t offset, uint64_t size);
    void render_pass_encoder_draw(WGPURenderPassEncoder renderPassEncoder, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance);
    void render_pass_encoder_draw_indexed(WGPURenderPassEncoder renderPassEncoder, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int32_t baseVertex, uint32_t firstInstance);
    void render_pass_encoder_draw_indexed_indirect(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer indirectBuffer, uint64_t indirectOffset);
    void render_pass_encoder_draw_indirect(WGPURenderPassEncoder renderPassEncoder, WGPUBuffer indirectBuffer, uint64_t indirectOffset);
    void render_pass_encoder_end(WGPURenderPassEncoder renderPassEncoder);
    void render_pass_encoder_release(WGPURenderPassEncoder renderPassEncoder);
    ...
    // Pipelines
    WGPUPipelineLayout create_pipeline_layout(const std::vector<WGPUBindGroupLayout>& bind_group_layouts, char const* label = nullptr);
    ...
    WGPUQuerySet create_query_set(uint8_t maximum_query_sets, char const* label = nullptr);

    void push_debug_group(WGPURenderPassEncoder render_pass, char const* label = nullptr);
    void push_debug_group(WGPUComputePassEncoder compute_pass, char const* label = nullptr);
