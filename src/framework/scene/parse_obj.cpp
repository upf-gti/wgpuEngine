    ...
    for (size_t s = 0; s < shapes.size(); s++) {
        ...
        if (!materials.empty()) {
            ...
            if (materials[material_id].diffuse_texname.empty()) {
                new_surface->set_material_color(glm::vec4(materials[material_id].diffuse[0], materials[material_id].diffuse[1], materials[material_id].diffuse[2], 1.0f));
                new_surface->set_material_shader(RendererStorage::get_shader("data/shaders/mesh_color.wgsl", new_surface->get_material()));
            }
            else {
                new_surface->set_material_diffuse(RendererStorage::get_texture(obj_path_fs.parent_path().string() + "/" + materials[material_id].diffuse_texname, true));
