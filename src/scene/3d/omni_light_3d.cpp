...
void OmniLight3D::render()
{
    ...
    if (debug_material) {
        ...
        Renderer::instance->add_renderable(debug_mesh_h->get_mesh(), get_global_model() * debug_mesh_h->get_global_model());
    }
#endif
...
void OmniLight3D::create_debug_meshes()
{
    ...
    debug_material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::mesh_forward::source, shaders::mesh_forward::path, shaders::mesh_forward::libraries, debug_material));
