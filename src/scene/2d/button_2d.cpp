...
#include "scene/2d/container_2d.h"
...
Button2D::Button2D(const std::string& signal, const sButtonDescription& desc) : Panel2D(signal, desc.path, desc.position, desc.size, desc.flags, desc.color)
{
    ...
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_button::source, shaders::ui_button::path, shaders::ui_button::libraries, material));
...
TextureButton2D::TextureButton2D(const std::string& signal, const sButtonDescription& desc) : Button2D(signal, desc)
{
    ...
    material->set_transparency_type(ALPHA_BLEND);
    material->set_priority(class_type);
    material->set_diffuse_texture(desc.path.size() ? RenderStorage::get_texture(desc.path, TEXTURE_STORAGE_UI) : nullptr);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_button::source, shaders::ui_button::path, shaders::ui_button::libraries, material));
...
ConfirmButton2D::ConfirmButton2D(const std::string& signal, const sButtonDescription& desc) : TextureButton2D(signal, desc)
{
...
void ConfirmButton2D::update(float delta_time)
{
    ...
    if (confirm_pending) {
        ...
        if (confirm_timer > 2.0f) {
            ...
            material->set_diffuse_texture(RenderStorage::get_texture(texture_path, TEXTURE_STORAGE_UI));
...
ButtonSubmenu2D::ButtonSubmenu2D(const std::string& signal, const sButtonDescription& desc) : TextureButton2D(signal, desc)
{
    class_type = Node2DClassType::SUBMENU;

    box = new HContainer2D("submenu_h_box_" + signal, { 0.0f, 0.0f });
...
ButtonSelector2D::ButtonSelector2D(const std::string& signal, const sButtonDescription& desc) : TextureButton2D(signal, desc)
{
    class_type = Node2DClassType::SELECTOR;

    box = new CircleContainer2D("circle_container", glm::vec2(0.0f, size.y + GROUP_MARGIN));
    box->set_visibility(false);
...
ComboButtons2D::ComboButtons2D(const std::string& name, const glm::vec2& pos, uint32_t flags, const Color& color) : HContainer2D(name, pos, flags, color)
{
