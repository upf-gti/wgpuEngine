...
Slider2D::Slider2D(const std::string& sg, const sSliderDescription& desc) : Panel2D(sg, desc.position, desc.size, desc.flags)
{
    data = desc.p_data;
    mode = desc.mode;
}
...
void Slider2D::process_wheel_joystick(T wheel_multiplier, T joystick_multiplier)
{
    ...
    T dt = static_cast<T>(XRManager::get_singleton()->get_thumbstick_value(HAND_RIGHT).y) * joystick_multiplier;

    if (glm::abs(dt) > 0.01f) {
        Node::emit_signal(name + "@stick_moved", dt);
    }

    T wheel_dt = static_cast<T>(InputManager::get_singleton()->get_mouse_wheel_delta()) * wheel_multiplier;
...
FloatSlider2D::FloatSlider2D(const std::string& sg, const sSliderDescription& desc) : Slider2D(sg, desc), original_value(desc.fvalue), current_value(desc.fvalue), min_value(desc.fvalue_min), max_value(desc.fvalue_max), precision(desc.precision)
{
    ...
    material->set_diffuse_texture(desc.path.size() > 0 ? RenderStorage::get_texture(desc.path, TEXTURE_STORAGE_UI) : nullptr);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_slider::source, shaders::ui_slider::path, shaders::ui_slider::libraries, material));
    ...
    Node::bind(name + "@changed", (FuncFloat)[&](const std::string& signal, float value) { set_value(value); });
...
bool FloatSlider2D::on_input(sInputData data)
{
    ...
    if (data.is_pressed) {
        ...
        if (std::abs(current_value - min_value) < 1e-4f) {
            current_value = min_value;
        } else if (std::abs(current_value - max_value) < 1e-4f) {
...
IntSlider2D::IntSlider2D(const std::string& sg, const sSliderDescription& desc) : Slider2D(sg, desc), original_value(desc.ivalue), current_value(desc.ivalue), min_value(desc.ivalue_min), max_value(desc.ivalue_max)
{
    bool is_horizontal = (mode == SliderMode::HORIZONTAL);
    ...
    Node::bind(name + "@changed", (FuncInt)[&](const std::string& signal, int value) { set_value(value); });
