        ...
        Node2D(name, p, s, flags), color(c)
{
    ...
    if (image_path.size()) {
        material->set_diffuse_texture(RenderStorage::get_texture(image_path, TEXTURE_STORAGE_UI));
    }

    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_panel::source, shaders::ui_panel::path, shaders::ui_panel::libraries, material));
    ...
    RenderStorage::register_ui_widget(webgpu_context, material->get_shader_ref(), quad_mesh, ui_data, 3);
...
{
    auto webgpu_context = Renderer::instance->get_webgpu_context();
    RenderStorage::delete_ui_widget(webgpu_context, quad_mesh);
...
bool Panel2D::was_input_pressed()
{
    ...
    bool is_2d = material->get_is_2D() || (!XRManager::get_singleton()->is_xr_available());

    if (is_2d) {
        return InputManager::get_singleton()->was_mouse_pressed(GLFW_MOUSE_BUTTON_LEFT);
    } else {
        return XRManager::get_singleton()->was_button_pressed(XR_BUTTON_A) || XRManager::get_singleton()->was_trigger_pressed(HAND_RIGHT);
    }
...
bool Panel2D::was_input_released()
{
    ...
    if (is_2d) {
        return InputManager::get_singleton()->was_mouse_released(GLFW_MOUSE_BUTTON_LEFT);
    } else {
        return XRManager::get_singleton()->was_button_released(XR_BUTTON_A) || XRManager::get_singleton()->was_trigger_released(HAND_RIGHT);
    }
...
bool Panel2D::is_input_pressed()
{
    ...
    if (is_2d) {
        return InputManager::get_singleton()->is_mouse_pressed(GLFW_MOUSE_BUTTON_LEFT);
    } else {
        return XRManager::get_singleton()->is_button_pressed(XR_BUTTON_A) || XRManager::get_singleton()->is_trigger_pressed(HAND_RIGHT);
    }
...
sInputData Panel2D::get_input_data(bool ignore_focus)
{
    ...
    if (material->get_is_2D()) {
        const glm::vec2& mouse_pos = InputManager::get_singleton()->get_mouse_position();
        ...
        if (XRManager::get_singleton()->is_xr_available()) {
            ray_origin = XRManager::get_singleton()->get_controller_position(HAND_RIGHT, POSE_AIM);
            glm::mat4x4 select_hand_pose = XRManager::get_singleton()->get_controller_pose(HAND_RIGHT, POSE_AIM);
            ray_direction = get_front(select_hand_pose);
        } else {
            Camera* camera = SimulationManager::get_singleton()->get_main_camera();
            glm::vec3 ray_dir = camera->screen_to_ray(InputManager::get_singleton()->get_mouse_position());
...
bool Panel2D::on_input(sInputData data)
{
    ...
    if (data.was_hovered) {
        XRManager::get_singleton()->vibrate_hand(HAND_RIGHT, HOVER_HAPTIC_AMPLITUDE, HOVER_HAPTIC_DURATION);
    }
...
{
    auto webgpu_context = Renderer::instance->get_webgpu_context();
    RenderStorage::update_ui_widget(webgpu_context, quad_mesh, ui_data);
}
        ...
        Panel2D(name, p, s, flags)
{
    ...
    material->set_diffuse_texture(RenderStorage::get_texture(image_path, TEXTURE_STORAGE_UI));
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_texture::source, shaders::ui_texture::path, shaders::ui_texture::libraries, material));

    quad_mesh->set_surface_material_override(quad_mesh->get_surface(0), material);

    auto webgpu_context = Renderer::instance->get_webgpu_context();
    RenderStorage::register_ui_widget(webgpu_context, material->get_shader_ref(), quad_mesh, ui_data, 3, true);
}
        ...
        Panel2D(name, image_path, p, s, flags, c)
{
    ...
    if (XRManager::get_singleton()->is_xr_available() && (flags & CURVED_PANEL)) {
    ...
    material->set_type(MATERIAL_UI);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_xr_panel::source, shaders::ui_xr_panel::path, shaders::ui_xr_panel::libraries, material));

    // Fullscreen in 2d mode only!
    fullscreen = !XRManager::get_singleton()->is_xr_available() && (flags & Node2DFlags::FULLSCREEN);
...
{
    if (!XRManager::get_singleton()->is_xr_available() || !(parameter_flags & CURVED_PANEL)) {
...
{
    XRPanel* new_button = new XRPanel(signal, texture_path, { 0.0f, 0.0f }, size, Node2DFlags::CURVED_PANEL);
        ...
        Panel2D("no_signal", pos, { 1.0f, 1.0f }, flags)
{
    ...
    material->set_priority(class_type);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_text_shadow::source, shaders::ui_text_shadow::path, shaders::ui_text_shadow::libraries, material));
...
void Text2D::update(float delta_time)
{
    ...
    // Set Text entity in place
    {
        ...
        t.scale(glm::vec3(scale.x, -scale.y, 1.0f)); // Hack to invert text mesh in 2d
...
bool Text2D::on_input(sInputData data)
{
    ...
    if (data.was_released) {
        if (!on_pressed()) {
            ...
            if (parameter_flags & Node2DFlags::TEXT_SELECTABLE) {
                selected = this;
            }
        }
    }
    ...
    {
        float dt = XRManager::get_singleton()->get_thumbstick_value(HAND_RIGHT).y;

        if (glm::abs(dt) > 0.01f) {
            Node::emit_signal(name + "@stick_moved", dt * 0.1f);
        }

        float wheel_dt = InputManager::get_singleton()->get_mouse_wheel_delta();
        ...
        Panel2D(sg, pos, size, flags, c)
{
    ...
    material->set_priority(class_type);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_color_picker::source, shaders::ui_color_picker::path, shaders::ui_color_picker::libraries, material));
