...
Node2D::Node2D(const std::string& n, const glm::vec2& p, const glm::vec2& s, uint32_t parameter_flags) : size(s)
{
    ...
    if ((parameter_flags & Node2DFlags::CREATE_3D) && XRManager::get_singleton()->is_xr_available()) {
...
void Node2D::set_xr_transform(const Transform& transform)
{
    if (xr_viewport_3d) {
        ...
        assert(0 && "Setting transform in non-XR mode! Use Node2DFlags::CREATE_3D flag when creating Node2D.");
    }
}
