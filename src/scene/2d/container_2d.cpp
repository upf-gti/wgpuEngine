...
Container2D::Container2D(const std::string& name, const glm::vec2& pos, const glm::vec2& s, uint32_t flags, const Color& col) : Panel2D(name, pos, s, flags, col)
{
...
HContainer2D::HContainer2D(const std::string& name, const glm::vec2& pos, uint32_t flags, const Color& color) : Container2D(name, pos, { 0.0f, 0.0f }, flags, color)
{
...
VContainer2D::VContainer2D(const std::string& name, const glm::vec2& pos, uint32_t flags, const Color& color) : Container2D(name, pos, { 0.0f, 0.0f }, flags, color)
{
...
CircleContainer2D::CircleContainer2D(const std::string& name, const glm::vec2& pos, uint32_t flags, const Color& color) : Container2D(name, pos, { 0.0f, 0.0f }, flags, color)
{
    ...
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_selector::source, shaders::ui_selector::path, shaders::ui_selector::libraries, material));
...
void CircleContainer2D::update(float delta_time)
{
    ...
    glm::vec2 axis_value = XRManager::get_singleton()->get_thumbstick_value(eXRHand::HAND_LEFT);
...
ItemGroup2D::ItemGroup2D(const std::string& name, uint32_t flags, const glm::vec2& pos, const Color& color) : HContainer2D(name, pos, flags, color)
{
    ...
    material->set_transparency_type(ALPHA_BLEND);
    material->set_depth_read_write(false);
    material->set_shader(RenderStorage::get_singleton()->get_shader_from_source(shaders::ui_group::source, shaders::ui_group::path, shaders::ui_group::libraries, material));
...
ImageLabel2D::ImageLabel2D(const std::string& p_text, const std::string& image_path, uint8_t mask, const glm::vec2& scale, float text_scale, const glm::vec2& p) : HContainer2D(p_text + "_box", p), mask(mask)
{
