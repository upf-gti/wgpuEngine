...
{
    Animation* animation = RenderStorage::get_animation(animation_name);
    if (!animation) {
...
void AnimationPlayer::generate_track_data()
{
    ...
    Animation* animation = RenderStorage::get_animation(current_animation_name);
...
void AnimationPlayer::render_gui()
{
    if (ImGui::BeginCombo("##current", current_animation_name.c_str())) {
        for (auto& instance : RenderStorage::animations) {
